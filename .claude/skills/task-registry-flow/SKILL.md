...
Active plans must declare `[[behaviors]]` in the plan manifest with `gap_id`, `polarity`, and typed `[[behaviors.verifiers]]` entries. Each implementation gap needs at least one `positive` and one `negative` behavior. Each task lists `behavior_ids`. Marking a task `completed` runs those verifiers and fails if checks fail.
